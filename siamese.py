...
#        template_z, templates_x = create_siamese_train(x_crops, z_crops, h=[11, 5, 3, 3, 3],w=[11, 5, 3, 3, 3],num=[96, 128, 96, 96, 32])
        ...
        template_z,templates_x = create_siamese_train(z_crops,x_crops,17,49,8,24)
...
#		# extend template_z to match the triple scaled feature map of x
        ...
        print("shape of templates_z:", templates_z.get_shape().as_list())#[24, 17, 17, 32]
        print("shape of templates_x:", templates_x.get_shape().as_list())#[24, 49, 49, 32]
#		
...
#        # resize to final_score_sz双线性插值放大到255
        ...
        print("shape of big score map:", scores_up.get_shape().as_list())#[24, 255, 255, 1]
#		
...
#scores = self._match_templates_train(templates_z=size[24, 17, 17, 32], templates_x=size[24, 49, 49, 32])    
    def _match_templates_train(self, net_z, net_x):#互相关层
		...
		# final is [B, Hf, Wf, C]
		#
        net_final = tf.reduce_mean(net_final, axis=3, keep_dims = True)
        ...
        return net_final# final is [B, Hf, Wf, 1]
